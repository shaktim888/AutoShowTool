...
var Zorder = [
	bg = {name:"bg",IMG:""},
	screen_bottom = {name:"screen",IMG:""},
	role = {name:"role",IMG:""},
	label = {name:"label",IMG:""},
	effect = {name:"effect",IMG:""}
]

let QMoHuanStyle = function(i,o,work,isL){
	this.inputPath = i
	this.outputPath = o
	this.workPath = work
	this.isL = isL
...
function genQMoHuanStyle(input,o){
	var index = 0
...
function getConponentParams(item,itemInfo,dev,id,lan){
	let t = {
		"55":{height:1242,width:2208},
		"65":{height:1242,width:2688}
	}
	...
	if(item.name == "bg" || item.name == "role"){
		...
		item.IMG = path.join(this.workPath,dev,item.name,lan,this.labelArr[id])
	}
